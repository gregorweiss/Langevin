...
template<typename TP>
Argument<TP>::Argument(Argument<TP> const &cp) :
        pos_(cp.pos_),
        nargs_(cp.nargs_),
...
template<typename TP>
Argument<TP>::Argument(const std::string &sid, TP const &sval, int const &nargs) :
        pos_(0),
        nargs_(nargs),
...
template<typename TP>
Argument<TP>::Argument(const std::string &sid, const std::string &sid2, TP const &sval, int const &nargs) :
        pos_(0),
        nargs_(nargs),
        id_(new std::vector<std::string>(1, sid)),
        data_(new TP[std::max<int>(this->nargs_, 1)]),
...
template<typename TP>
Argument<TP>::Argument(int const &pos, const std::string &sid, TP const &sval) :
        pos_(pos),
        nargs_(0),
        id_(new std::vector<std::string>(1, sid)),
        data_(new TP[std::max<int>(this->nargs_, 1)]),
...
template<typename TP>
Argument<TP> &Argument<TP>::operator=(Argument<TP> const &cp) {
    if (this == &cp) {
        return *this;
    }
...
template<typename TP>
std::vector<std::string> &Argument<TP>::id() {
    return *this->id_;
}

template<typename TP>
std::vector<std::string> &Argument<TP>::id() const {
    return *this->id_;
}

template<typename TP>
std::string &Argument<TP>::id(unsigned const &i) {
    return (*this->id_)[i];
}

template<typename TP>
std::string &Argument<TP>::id(unsigned const &i) const {
    return (*this->id_)[i];
}

template<typename TP>
int Argument<TP>::add_id(const std::string &add) {
    this->id_->push_back(add);
    return 0;
}

template<typename TP>
std::string &Argument<TP>::description() {
    return this->descr_;
}

template<typename TP>
std::string &Argument<TP>::description() const {
    return this->descr_;
}

template<typename TP>
int &Argument<TP>::size() {
    return this->nargs_;
}

template<typename TP>
int const &Argument<TP>::size() const {
    return this->nargs_;
}
...
template<typename TP>
int const &Argument<TP>::pos() const {
    return this->pos_;
}

template<typename TP>
bool Argument<TP>::is(const std::string &str) {
    bool result = 0;
...
template<typename TP>
bool Argument<TP>::is_parsed() {
    return parsed;
}

template<typename TP>
void Argument<TP>::count_nargs_(int const &i, int argc, char *argv[]) {
 ...
 */
template<typename TP>
int Argument<TP>::parse(int argc, char *argv[]) {
    if (this->pos_ == 0) {
        std::stringstream inss(std::stringstream::in | std::stringstream::out);
...
template<typename TP>
TP &Argument<TP>::val(int const &i) {
    assert(i >= 0 && i < std::max<int>(this->nargs_, 1));
    return this->data_[i];
}

template<typename TP>
TP const &Argument<TP>::val(int const &i) const {
    assert(i >= 0 && i < std::max<int>(this->nargs_, 1));
    return this->data_[i];
}
